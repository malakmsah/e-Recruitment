 ...
 */
public class RecruitersDao extends ConnectionDao {

    private final RecruiterAdressDao recruiterAdressDao = new RecruiterAdressDao();

    public Integer getMaxIdForRecruiters()
            throws Exception {
        int maxId = 0;
        try {
    ...
    public int insertRecruiters(Recruiters recruiters) throws Exception {
        int maxID = getMaxIdForRecruiters();
        try {
            Connection conn = getConnection();
            ...
            ps.setString(6, recruiters.getPhone());
            ps.setString(7, recruiters.getEmail());
            ps.setString(8, recruiters.getAbout());
            ps.setInt(9, recruiters.getNumberOfEmployee());
    ...
    public void updateRecruiters(Recruiters recruiters) throws Exception {
        try {
            Connection conn = getConnection();
            ...
            ps.setString(5, recruiters.getPhone());
            ps.setString(6, recruiters.getEmail());
            ps.setString(7, recruiters.getAbout());
            ps.setInt(8, recruiters.getNumberOfEmployee());
    ...
    public Recruiters getRecruiters(int id) throws Exception {
        try {
            ...
            while (rs.next()) {
                ...
                recruiters.setPhone(rs.getString("PHONE"));
                recruiters.setNumberOfEmployee(rs.getInt("NUMBER_OF_EMPLOYEE"));
                recruiters.setCreatedAt(rs.getTimestamp("CREATED_AT"));
                recruiters.setFoundedAt(rs.getTimestamp("FOUNDED_AT"));
            }
    ...
    public static void main(String[] args) {
        try {
            RecruitersDao dao = new RecruitersDao();
            //ArrayList<Event> events = dao.buildEvents();
        } catch (Exception ex) {
            Logger.getLogger(RecruitersDao.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
